...
PressureSensor::PressureSensor()
: raw_pressure(0), raw_temperature(0), current_pressure(NAN), current_temperature(NAN) {
...
void PressureSensor::callback_sample_done(void) {
    ...
    if (sample_cmd == CMD_SAMPLE_PRESSURE) {
        ...
        current_temperature = (float) comp_temperature / 100.0f;
        
        end = start;
        start = systemClock.getUnixTimestamp();
        
        last_pressure= current_pressure;
        current_pressure = (float) comp_pressure / 100000.0f;
        FULL_BARRIER();
        state = READY;
    } else
        assert(0);
...
float PressureSensor::get_atm_pressure() {
    return atm_pressure;
}
...
float PressureSensor::get_rate() {
    
    float second = difftime(end, start);
    
    if(second == 0){
        return 0;
    }
    
    return ((current_pressure - last_pressure) * 60) / second;
    
}

void PressureSensor::capture_data() {
    pressureSensor.sample(DepthSampling::OSR_4096);
    pressureSensor.wait();
}

float PressureSensor::get_pressure_bar() {
    assert(state == READY && !isnan(current_pressure));

    //in case the user jump directly in the water and the computer is sleeping, 
    //there are not a big difference in turn of pressure between the surface and a few centimeter below the surface
    if (atm_pressure == 0.0) {
        last_pressure = current_pressure;
        atm_pressure = current_pressure;
